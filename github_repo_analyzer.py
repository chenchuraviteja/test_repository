...
class GitHubRepoAnalyzer:
    ...
    def check_python_libraries(self, owner, repo):
        """Check if a Python repository has New Relic dependencies"""
        has_newrelic = False
        
        # Check for New Relic in requirements.txt
        req_content = self.download_file(owner, repo, "requirements.txt")
        if req_content:
            if re.search(r'newrelic[=><~]', req_content, re.IGNORECASE) or \
            re.search(r'^newrelic$', req_content, re.IGNORECASE | re.MULTILINE):
                has_newrelic = True
        
        # Check for New Relic in setup.py if not found in requirements.txt
        if not has_newrelic:
            setup_content = self.download_file(owner, repo, "setup.py")
            if setup_content and "newrelic" in setup_content.lower():
                if re.search(r'[\'"](install_requires|requires)[\'"]: .*?[\'"](newrelic)', setup_content, re.IGNORECASE):
                    has_newrelic = True
        
        # Check for New Relic in Pipfile if not found yet
        if not has_newrelic:
            pipfile_content = self.download_file(owner, repo, "Pipfile")
            if pipfile_content and "newrelic" in pipfile_content.lower():
                has_newrelic = True
        
        # Check for New Relic in pyproject.toml if not found yet
        if not has_newrelic:
            pyproject_content = self.download_file(owner, repo, "pyproject.toml")
            if pyproject_content and "newrelic" in pyproject_content.lower():
                if re.search(r'(dependencies|requires) *= *\[.*[\'"](newrelic)', pyproject_content, re.IGNORECASE):
                    has_newrelic = True
