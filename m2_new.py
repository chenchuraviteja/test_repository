...
def process_row(row):
    """Process a single row to extract ASV info from GitHub repositories.
    
    Args:
        row: Dictionary containing repository information with at least a 'Repository' key
    ...
    Returns:
        ...
        - Language and deployment details
    """
    try:
        ...
        try:
            ...
            # Download and process Bogiefile
            bogiefile_content = download_file(owner, repo, "Bogiefile")

            if bogiefile_content is None:
            ...
            # Try to extract ASV from YAML first
            asv_value = extract_asv_from_yaml(bogiefile_content)
            
            # Fallback to regex if YAML parsing fails
            if asv_value is None:
                asv_value = extract_asv_via_regex(bogiefile_content)

            if asv_value is not None:
                result["ASV"] = asv_value

            # Extract flavor and language from Bogiefile
            extract_flavor_and_language(bogiefile_content, result)

            # Check for monitoring frameworks
            check_monitoring_frameworks(bogiefile_content, result)

            # Perform language-specific analysis
            if result["Language"]:
                analyze_by_language(owner, repo, result)
...
def check_file_exists(owner, repo, file_path):
    ...
    try:
        ...
        print(f"Error checking file {file_path} in {owner}/{repo}: {e}")
        return False
    
def detect_actual_language(owner, repo):
    """Detect the primary language by checking for characteristic files"""
    language_indicators = [
        ("pom.xml", "java"),
        ("build.gradle", "java"),
        ("package.json", "node"),
    ...
    for file_name, lang in language_indicators:
        if check_file_exists(owner, repo, file_name):
            return lang
    
    # Fallback: Check for file extensions in the root directory
    try:
        contents = requests.get(
            f"{api_base_url}/repos/{owner}/{repo}/contents/",
            headers=headers
        ).json()
        ...
        if extensions_count:
            # Return the language associated with the most common extension
            common_ext = max(extensions_count.items(), key=lambda x: x[1])[0]
            return {
                '.java': 'java',
                '.py': 'python',
                '.js': 'node',
                ...
                '.rb': 'ruby',
                '.clj': 'clojure'
            }.get(common_ext, "unknown")
            
    except Exception as e:
        print(f"Error detecting language from files: {e}")
    
    return None
    
def extract_asv_from_yaml(bogiefile_content):
    """Extract ASV value from YAML content"""
    try:
        ...
        if bogie_yaml and isinstance(bogie_yaml, dict):
            if "bogie" in bogie_yaml and isinstance(bogie_yaml["bogie"], dict):
                return bogie_yaml["bogie"].get("asv")
            elif "vars" in bogie_yaml and isinstance(bogie_yaml["vars"], dict):
                return bogie_yaml["vars"].get("asv")
    except Exception as yaml_e:
        print(f"Error parsing YAML: {yaml_e}")
    return None

def extract_asv_via_regex(bogiefile_content):
    """Fallback to regex ASV extraction if YAML parsing fails"""
    try:
        asv_match = re.search(pattern, bogiefile_content, re.IGNORECASE)
        return asv_match.group(2).strip() if asv_match else None
    except Exception as regex_e:
        print(f"Error in regex ASV extraction: {regex_e}")
    return None

def extract_flavor_and_language(bogiefile_content, result):
    """Extract flavor and language from Bogiefile content"""
    try:
        bogie_yaml = read_yaml(bogiefile_content)
        if bogie_yaml and isinstance(bogie_yaml, dict):
        ...
        print(f"Error extracting flavor/language: {e}")

def check_monitoring_frameworks(bogiefile_content, result):
    """Check for monitoring framework indicators in Bogiefile"""
...
def analyze_by_language(owner, repo, result):
    """Perform language-specific analysis with docker handling"""
    lang = result["Language"].lower() if result["Language"] else None
    
    # Handle docker case first
    if lang == "docker":
        actual_lang = detect_actual_language(owner, repo)
        if actual_lang:
            result["Language"] = actual_lang
            print(f"Converted docker to {actual_lang} for {owner}/{repo}")
        return
    
    # Normalize other language names
    language_map = {
        "npm": "node",
        "yarn": "node",
        "maven": "java",
    ...
    # Special case for Go
    if result["Language"] and result["Language"].lower() in ["go", "golang"]:
        result["MANUAL_CASE"] = True
        return
    
    # Only proceed with framework detection if we have a valid language
    if result["Language"]:
        code_search_results = search_code_for_patterns(owner, repo, result["Language"])
        result.update({
            "has_newrelic": code_search_results["has_newrelic"],
            "has_micrometer": code_search_results["has_micrometer"],
            "has_prometheus": code_search_results["has_prometheus"],
            "has_aws_lambda_powertools": code_search_results["has_aws_lambda_powertools"],
            "monitoring_frameworks": ",".join(code_search_results["monitoring_frameworks"]) 
                if code_search_results["monitoring_frameworks"] else None,
            "logging_frameworks": ",".join(code_search_results["logging_frameworks"]) 
                if code_search_results["logging_frameworks"] else None
        })
        
        # Update flags based on findings
        if any([result["has_newrelic"], result["has_micrometer"], 
               result["has_prometheus"], result["has_aws_lambda_powertools"]]):
            result["MANUAL_CASE"] = True
        ...
        if any([result["has_micrometer"], result["has_prometheus"], 
