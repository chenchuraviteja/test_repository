...
def write_result(result):
    with write_lock:
        with open(OUTPUT_FILE, 'a', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=['github_url', 'asv'])
...
def process_csv_and_extract_asv_parallel(csv_file, max_workers=20):
    ...
    def process_and_write(index, github_url):
        result = process_repo(index, github_url)
        write_result(result)

    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(process_and_write, idx, re.sub(r"/tree/.*$", "", url))
    ...
    print("✅ Finished! Incremental results saved to 'asv_results.csv'")
